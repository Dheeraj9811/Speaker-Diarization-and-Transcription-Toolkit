...
# Function to set the key in the .env file
def set_key(value,file_path='.env', key='use_auth_token'):
    key_value_string = f'{key}: "{value}"'
    with open(file_path, 'a') as f:  
        f.write(f'{key_value_string}\n')
...
def home():
    ...
    my_expander4 = st.expander("**How to get your model access token to use Segment Audio according to user**")
    my_expander4.write('''To use the Segment Audio feature, you need to obtain an access token.\n
        **Follow these steps to get your access token**
        ...
        3. Create access token at hf.co/settings/tokens. Copy the token and paste it in the 'Enter your key' field. Click on 'Save Key' to save the token. You can now use the Segment Audio feature.''')

    user_key = st.text_input("Enter your Token", type="password")
    if st.button("Save Key"):
        if user_key:
            # Write the key to the .env file
            set_key(key='use_auth_token', value=user_key)
        else:
...
def main():
    ...
    if st.session_state['showarning'] == False:
        ...
        # if .env file exist then delete it
...
if __name__ == "__main__":
    ...
    if not os.path.exists("uploads"):
        os.makedirs("uploads")
    # if .env file not exist then create it
